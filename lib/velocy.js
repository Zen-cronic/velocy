...
class Router {
  ...
  addRoute(httpMethod, routePath, requestHandlers) {
    ...
    console.log("added requrestHandlers: ", requestHandlers);
    currentNode.handler[httpMethod] = requestHandlers;
    // [[]]
    // console.log(currentNode.handler[httpMethod]);
  }
  ...
  #printNode(node, prefix, level = 0, prefixSymbol = "") {
    ...
    for (const [method, handlers] of Object.entries(node.handler)) {
      ...
      //   const handlerNames = handlers[0].map(
      const handlerNames = handlers.map((handler) => {
        // console.log("mapped handler: ", handler);
      ...
      console.log(`${indentation}    └─ [${method}] ↠  ${handlerNames}`);
    }
  ...
  async #handleMiddleware(middlewareFunctions, req, res, index = 0) {
    const currFunc = middlewareFunctions[index];
    ...
    //last elem is main handler
    if (index === middlewareFunctions.length - 1) {
      await currFunc(req, res);
      return null;
    }
    index++;
    ...
    await currFunc(
      ...
      res,
      async () =>
        await this.#handleMiddleware(middlewareFunctions, req, res, index)
    );
  }
  ...
  async handleRequest(nativeReq, nativeRes) {
    ...
    req.queryParams = new URLSearchParams(
    ...
    //why in arr [[]]? sol: double ...rest in httpMethod() and addRoute()
    const routeHandlerFunctions = routeHandler.requestHandlers;
    // const routeHandlersLength = routeHandlerFunctions.length;
  

    // const manageMiddleware = async (req, res, index) => {
    //   const currFunc = routeHandlerFunctions[index];
    //   if (typeof currFunc !== "function") {
    //     res.writeHead(404);
    //     res.end("Route Not Found");
    //   }
    ...
    //   //last elem is main handler
    //   if (index === routeHandlersLength - 1) {
    //     await currFunc(req, res);
    //     return null;
    //   }

    //   index++;
    //   // if (index === routeHandlersLength) {
    //   //   return null;
    //   // }

    //   await currFunc(req, res, () => manageMiddleware(req, res, index));
    // };

    // await manageMiddleware(req, res, 0);

    await this.#handleMiddleware(routeHandlerFunctions, req, res);
  }
  ...
  #findRouteHandler(httpMethod, routePath) {
    ...
    //requestHandlers an array of middleware + main handler
    return {
      requestHandlers: currentNode.handler[httpMethod],
      extractedParams,
    };
  }
  ...
  get(routePath, ...requestHandlers) {
    this.addRoute("GET", routePath, requestHandlers);
    return this;
  }
...
class Response {
  ...
  write(chunk) {
    this.#nativeResponse.write(chunk);
  }
...
class SimpleRouter {
  ...
  addRoute(httpMethod, routePath, requestHandler) {
    const route =
      this.routes[routePath] || (this.routes[routePath] = Object.create(null));
